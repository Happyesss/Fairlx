  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="start"
        side="top"
        className="w-44 bg-popover border-border text-popover-foreground shadow-xl rounded-xl p-1"
      >
        {AGENT_SESSION_MODES.map((mode) => {
          ...
          return (
            <DropdownMenuItem
              ...
              title={mode.hint}
              className="flex items-center justify-between text-xs font-medium cursor-pointer rounded-lg px-2.5 py-1.5 hover:bg-muted transition-colors group"
            >
              <div className="flex items-center gap-2.5">
                <Icon className={cn("size-3.5 shrink-0", config.itemIconColor)} />
                <span className="text-foreground font-medium">{mode.label}</span>
              </div>
              {currentMode === mode.id && <Check className="size-3.5 text-foreground shrink-0" />}
            </DropdownMenuItem>
          );
        })}
      </DropdownMenuContent>
